...
export function BioDisplay() {
  ...
  return (
    <LoadingOverlay isLoading={isLoading}>
      {showComingSoon && <ComingSoon duration={1500} x={cursorPos.x} y={cursorPos.y + 20} />}
                ...
                {category.items.map((item) => (
                  <div key={item.id} className={styles.noteSection}>
                    <div className={styles.noteHeader}>
                      <Icon
                        icon={
                          item.completed ? 'mdi:checkbox-marked-circle-outline' : 'mdi:checkbox-blank-circle-outline'
                        }
                        width={'1.25rem'}
                        height={'1.25rem'}
                    ...
                    <span className={styles.noteItemDates}>{formatTime(item.start, item.end)}</span>
