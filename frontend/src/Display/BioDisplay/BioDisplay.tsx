...
export function BioDisplay() {
  ...
  return (
    <>
      {showComingSoon && <CommingSoon message="The 'back' feature is coming soon!" duration={1500} />}
      <div className={styles.bioDisplayContainer}>
        <div className={styles.notesActionRack}>
          ...
          <div>
            <Icon icon={'ion:share-outline'} width={'2rem'} height={'2rem'} onClick={handleBackClick} />
            <Icon icon={'system-uicons:circle-menu'} width={'2rem'} height={'2rem'} onClick={handleBackClick} />
          </div>
        </div>
                    ...
                    {note.NOTE_CONTENTS[subtitle].map((item) => (
                      <div className={styles.noteSection}>
                        <div className={styles.noteHeader}>
                          {item.completed ? (
                            <Icon icon={'mdi:checkbox-marked-circle-outline'} width={'1.25rem'} height={'1.25rem'} />
                          ) : (
                            <Icon icon={'mdi:checkbox-blank-circle-outline'} width={'1.25rem'} height={'1.25rem'} />
                          )}
                          <h4 className={styles.noteItemTitle}>{item.title}</h4>
                        </div>
                        <span className={styles.noteItemDates}>{item.dates}</span>
                        <p className={styles.noteItemDescription}>{item.description}</p>
                      </div>
                    ))}
                  </div>
        ...
        <div className={styles.actionFooter}>
          <Icon icon={'ph:list-checks-light'} width={'2rem'} height={'2rem'} onClick={handleBackClick} />
          <Icon icon={'ph:camera-light'} width={'2rem'} height={'2rem'} onClick={handleBackClick} />
          <Icon icon={'ph:pencil-circle-light'} width={'2rem'} height={'2rem'} onClick={handleBackClick} />
          <Icon icon={'ph:note-pencil-light'} width={'2rem'} height={'2rem'} onClick={handleBackClick} />
        </div>
      </div>
    </>
  );
